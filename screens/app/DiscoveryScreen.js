...
const DiscoveryScreen = () => {
  ...
  const renderImageItem = (source, name, location, age, isNew) => (
      <View style={styles.imageContainer}>
          ...
          <View style={styles.overlay}>
              <Text style={styles.overlayText}>{name}, {location}</Text>
  ...
  const loginUser  =  require('../../assets/images/onboarding/face_1.jpg');
  const nearbyUsers = [
      {
        id: '1',
        latitude: 37.785834,
  ...
  return (
    ...
    <ScrollView>
        ...
        <ScrollView 
            ...
            {renderImageItem(require('../../assets/images/onboarding/face_1.jpg'), 'Charlie', 'Dallas', '35', true)}
            {renderImageItem(require('../../assets/images/onboarding/face_2.jpg'), 'Alice', 'Chicago', '28', true)}
            {renderImageItem(require('../../assets/images/onboarding/face_3.jpg'), 'Jamie', 'NYC', '22', false)}
        </ScrollView>
        ...
        <View style={styles.interestsContainer}>
            {interests.map(interest => (
                <TouchableOpacity 
                    key={interest.id} 
                    style={[ styles.interestButton, selectedInterests.includes(interest.id) && styles.interestButtonSelected ]}
                    onPress={() => toggleInterest(interest.id)}
                >
                    <Icon name={interest.icon} size={24} color={selectedInterests.includes(interest.id) ? 'white' : 'black'} />
                    <Text bold style={[styles.interestLabel, selectedInterests.includes(interest.id) && styles.interestLabelSelected ]}>{interest.label}</Text>
                </TouchableOpacity>
            ))}
        </View>
