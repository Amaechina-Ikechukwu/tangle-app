...
import { GetLikeCount } from "../../services/Posts/api";
import { auth } from "../../firebase";
import SubscriptionSuggestion from "./SubscriptionSuggestion";

const DatingProfileCard = ({ item, navigation, from, index, currentUser }) => {
  const enlargeImage = () => {
    navigation.navigate({
      name: "View Image",
    ...
    <>
      <View style={styles.cardContainer}>
        ...
        <View style={styles.interestTag}>
          <Text style={{ fontWeight: 600 }}>{item?.type && item.type}</Text>
        </View>
        ...
        <View style={styles.iconContainer}>
          <LikeComponent
            postid={item.postid}
            liked={item.liked}
            isSubscribed={currentUser?.subscribed}
            isCurrentUserPost={item.author == currentUser?.userKey}
          />
          {from !== "comments" && (
      ...
      {index >= 3 && index % 3 == 0 && !currentUser?.subscribed && (
        <View style={{ paddingHorizontal: 10 }}>
          <SubscriptionSuggestion text={"have access to more features"} />
        </View>
      )}
    </>
  );
