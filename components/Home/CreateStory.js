...
export default function CreateStory({ image, height }) {
  ...
  const [currentUser, setStoryList, setStories] = useStore(
    useShallow((state) => [
      state.currentUser,
      state.setStoryList,
      state.setStories,
  ...
  const handleImageUpload = async () => {
    try {
      ...
      const posts = await GetStoryList({ token: auth?.currentUser?.uid });
      const stories = await GetStories({ token: auth?.currentUser?.uid });
      setPostContent("");
      setStoryList(posts);
      setStories(stories);
      navigation.goBack();
